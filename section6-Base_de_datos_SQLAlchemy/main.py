...
class AuthorORM(Base):
    ...
    #Un autor puede tener muchos post entonces hacemos la relacion (relacion uno a muchos):
...
def filter_by_tags(
    ...
    post_list=(
        select(PostORM)
        .options(
            selectinload(PostORM.tags),     #evitando el problema n+1 al serializar.
...
def get_post(post_id:int= Path(
    ...
    example=1
    ), include_content:bool = Query(default=True, description="Incluye contenido o no"), db: Session=Depends(get_db) ):
    
    post = db.get(PostORM,post_id)  #--> busqueda por primary keys.
    
    ##forma alternativa mas flexible:
...
def create_post(post: PostCreate, db:Session=Depends(get_db)):
    ...
    for tag in post.tags:
        ...
        if not tag_obj:
            ...
            db.add(tag_obj)
            db.flush()
        new_post.tags.append(tag_obj) #agregamos los tags a la tabla intermedia
    try:
        db.add(new_post) #agregamos a la base de datos
        db.commit()  #confirmar
        db.refresh(new_post) #. Traerme los valores finales.
                             # para que me cargue todo, por ej el id.
        return new_post 
    except IntegrityError:
        db.rollback()
        raise HTTPException(status_code=409, detail="El título ya existe, prueba con otro")
    except SQLAlchemyError:
        db.rollback()  #me deshaga todo
...
@app.delete("/posts/{post_id}", status_code=status.HTTP_204_NO_CONTENT) #Como no quiero devolver nada, solo pongo return y status code 204.
def delete_post(post_id:int,db:Session=Depends(get_db)):
    post = db.get(PostORM,post_id)
    if not post:
        raise HTTPException(status_code=404, detail="Post no encontrado")
    db.delete(post) #eliminamos de la base de datos
    db.commit()  #confirmar
    return 
