...
@router.get("/{post_id}", response_model=Union[PostPublic, PostSummary], response_description="Post encontrado") #Evala y se fija es de tipo PostPubli y sino, se fija si es de tipo PostSummary
...
def create_post(post: PostCreate, db:Session=Depends(get_db)):
    ...
    try:
        ...
        db.commit()  #confirmar
        db.refresh(post) #. Traerme los valores finales.
...
@router.put("/{post_id}", response_model= PostPublic, response_description="Post actualizado", response_model_exclude_none=True) #ecluye los none
...
@router.delete("/{post_id}", status_code=status.HTTP_204_NO_CONTENT) #Como no quiero devolver nada, solo pongo return y status code 204.
